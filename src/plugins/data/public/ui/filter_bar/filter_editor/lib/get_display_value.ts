...
import {
  IESQueryIndexPattern,
  IESQueryIndexPatternField,
} from '../../../../../../../platform/es_query/common/index_pattern';

function getValueFormatter(indexPattern?: IESQueryIndexPattern, key?: string) {
  ...
  if (!format && (indexPattern.fields as any).getByName) {
    ...
    const field: IESQueryIndexPatternField = (indexPattern.fields as any).getByName(key);
    if (!field) {
      throw new Error(
...
export function getDisplayValueFromFilter(
  filter: Filter,
  indexPatterns: IESQueryIndexPattern[]
): string {
